...
cl_local = zeros(N,ncases) ; force_coeff = zeros(10,ncases) ;
...
for icase = 1:ncases % calculate loads for the diferent aoa
    ...
    if ( sum(CDP,'all')>0 ) % add profile drag (estimated from experimental sections' data)
        ...
        for i = 1:N
            ...
            ck = CDP(1,2) + (CDP(2,2)-CDP(1,2))*ypos ;
            
            tot_cdp = tot_cdp + ( cd0 + ck*cl_y^2 )*chord(i)*h ;
...
%     angle_of_attack = aoa
%     CL = force_coeff(7,icase)
%     CMy = force_coeff(5,icase)
%     Cd_i = force_coeff(9,icase)
%     CD_p = force_coeff(10,icase)
%     CD = Cd_i + CD_p
